...
import static io.microsphere.rest.util.PathUtils.ENCODED_SLASH;
import static io.microsphere.rest.util.PathUtils.SLASH;
...
public interface URLUtils {
        ...
        if (encodeSlashInPath) {
            ...
            for (Map.Entry<String, ?> entry : templateValues.entrySet()) {
                ...
                if (value instanceof String) {
                    ...
                    value = replace(str, SLASH, ENCODED_SLASH);
                }
    ...
    static Map<String, List<String>> resolveParameters(String queryString) {
        if (isNotBlank(queryString)) {
            ...
            String[] queryParams = split(queryString, AND);
            if (queryParams != null) {
                for (String queryParam : queryParams) {
                    String[] paramNameAndValue = split(queryParam, EQUAL);
                    if (paramNameAndValue.length > 0) {
                        String paramName = paramNameAndValue[0];
                        String paramValue = paramNameAndValue.length > 1 ? paramNameAndValue[1] : EMPTY;
